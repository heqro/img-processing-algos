...
for step in range(0, len(dfs_dict.get(noise_get)), 21):
    ...
    for col_name in dfs_dict.get(noise_get).columns[1:]:
        ax.plot(img_indices, dfs_dict.get(noise_get)[col_name][step:(step + 21)],label=f'syn_{i}', marker='.',linestyle='dashed', markersize=10,lw=0.5)
        i += 1
    ...
    pp = PdfPages(f'005-{step}-p{p}.pdf')
    pp.savefig(fig)
    pp.close()
    # plt.show()
