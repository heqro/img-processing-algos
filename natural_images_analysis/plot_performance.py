...
noise_get = '0.15'
for step in range(0, len(dfs_dict.get(noise_get)), 20):
    img_indices = dfs_dict.get(noise_get)[dfs_dict.get(noise_get).columns[0]][step:(step + 20)]
    fig = plt.figure(figsize=(15, 6))
    ax = fig.add_subplot(1, 1, 1)
    i = 1
    for col_name in dfs_dict.get(noise_get).columns[1:]:
        ax.scatter(img_indices, dfs_dict.get(noise_get)[col_name][step:(step + 20)],label=f'syn_{i}')
        i += 1
