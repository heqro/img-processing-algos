...
def get_closest_point(data: DataFrame, height: float, width: float):
    ...
    return filtered_h_w.loc[filtered_h_w.apply(distance, axis=1, args=(height, width)).idxmin()]
...
def get_stoppage_coefficient(data: DataFrame, height: float, width: float, noise_std: float):
    ...
    """
    closest_point = get_closest_point(data, height, width)
